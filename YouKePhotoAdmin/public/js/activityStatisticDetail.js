$(function () {
    ActivityStatisticDetail.init();
});
var ActivityStatisticDetail = {
    datatitle: "日作品数",
    datatype: 1,
    canvasmethodName: "/AdminActivity/GetAdminActivityWorkData",
    init: function () {
        ActivityStatisticDetail.dataInit();
        $('#myTab a').click(function (e) {
            e.preventDefault()
            console.log($(this).attr("data-type"));
            if (ActivityStatisticDetail.datatype == $(this).attr("data-type")) {
                return false;
            }
            ActivityStatisticDetail.datatype = $(this).attr("data-type");
            if (ActivityStatisticDetail.datatype == 1) {
                ActivityStatisticDetail.datatitle = "日作品数";
                $(".trend_analyse_title").text(ActivityStatisticDetail.datatitle);
                ActivityStatisticDetail.canvasmethodName = "/AdminActivity/GetAdminActivityWorkData";
            } else if (ActivityStatisticDetail.datatype == 2) {
                ActivityStatisticDetail.datatitle = "日浏览数";
                $(".trend_analyse_title").text(ActivityStatisticDetail.datatitle);
                ActivityStatisticDetail.canvasmethodName = "/AdminActivity/GetAdminActivityBroswerInfo";
            }
            ActivityStatisticDetail.dataInit();
        })
    },
    ...
    GetAdminNewUserInfo: function (startTime, endTime) {
        //方法名
        var methodName = ActivityStatisticDetail.canvasmethodName;
        //获取数据
        var data = {
            StartDate: startTime,
            EndDate: endTime,
        ...
        SignRequest.set(methodName, data, function (data) {
            ...
            if (data.Data != null) {
                ...
                ActivityStatisticDetail.UpdateMessage_one(Data_list, data.Data);
            }
        }, function () {});
    },
    dataInit: function () {
        ...
        var end_time1 = moment().subtract(0, "days").format('YYYY-MM-D');
        ActivityStatisticDetail.GetAdminNewUserInfo(start_time, end_time1);
        //第一个日历
        var option = {
            locale: {
                fromLabel: '开始日期',
        ...
        $('#reportrange2 span').html(moment().subtract('days', 29).format('YYYY-MM-D') + ' - ' + moment().subtract(0, "days").format('YYYY-MM-D'));
        ...
        $('#reportrange2').daterangepicker(option, function (start, end1, label) {
            ...
            //获取新增会员统计
            ActivityStatisticDetail.GetAdminNewUserInfo(startTime, endTime)
        });
    },
    //更新图表一视图
    UpdateMessage_one: function (Data, ExData) {
        ActivityStatisticDetail.messageOneData = ExData;
        option = {
            title: {
                //text: '消息趋势分析'
            },
            ...
            legend: {
                data: [ActivityStatisticDetail.datatitle],
                bottom: 0,
            ...
            toolbox: {
                feature: {
                    ...
                    myTool_export: {
                        ...
                        onclick: function () {
                            console.log(ActivityStatisticDetail.messageOneData)
                            ActivityStatisticDetail.ExportQueryData(ActivityStatisticDetail.messageOneData);
                        }
                    },
                },
                width: 50,
            ...
            series: [{
                name: ActivityStatisticDetail.datatitle,
                type: 'line',
